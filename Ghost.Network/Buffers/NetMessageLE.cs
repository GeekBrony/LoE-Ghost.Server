...
{
    internal unsafe class NetMessageLE : INetMessage
    {
        ...
        public float ReadSingle()
        {
            ...
            if (m_bits_offset == 0)
                ...
                *(uint*)&value = (uint)((*(uint*)m_offset | (ulong)m_offset[sizeof(float)] << (sizeof(float) << 3)) >> m_bits_offset);
        ...
        public double ReadDouble()
        {
            ...
            if (m_bits_offset == 0)
                ...
                *(ulong*)&value = (*(ulong*)m_offset >> m_bits_offset) | ((ulong)m_offset[sizeof(double)] << ((sizeof(double) << 3) - m_bits_offset));
        ...
        {
            CheckRead(sizeof(Vector2));
            Vector2 value;
            if (m_bits_offset == 0)
                value = *(Vector2*)m_offset;
            else
                *(ulong*)&value = (*(ulong*)m_offset >> m_bits_offset) | ((ulong)m_offset[sizeof(ulong)] << ((sizeof(ulong) << 3) - m_bits_offset));
        ...
        {
            CheckRead(sizeof(Vector3));
            Vector3 value;
            if (m_bits_offset == 0)
                value = *(Vector3*)m_offset;
            else
            {
                var src = m_offset;
                var dest = (uint*)&value;
                *(ulong*)dest = (*(ulong*)src >> m_bits_offset) | ((ulong)src[sizeof(ulong)] << ((sizeof(ulong) << 3) - m_bits_offset));
                src += sizeof(ulong);
                dest[2] = (uint)((*(uint*)src | (ulong)src[sizeof(uint)] << (sizeof(uint) << 3)) >> m_bits_offset);
            }
        ...
        public void Read(INetMessage message, int bytes)
        {
            CheckRead(sizeof(byte) * bytes);
            if (m_bits_offset == 0)
                message.Write(m_segment.Buffer, (int)(m_segment.Offset + Position), bytes);
            else Copy(this, message, bytes);
            m_offset += bytes;
        }
        ...
        {
            CheckWrite(sizeof(Vector2));
            if (m_bits_offset == 0)
            {
                *(Vector2*)m_offset = value;
                m_offset += sizeof(Vector2);
            }
            else
            {
                ...
                *m_offset = (byte)((byte)(*m_offset & (byte.MaxValue << m_bits_offset)) | (temp >> (sizeof(ulong) << 3) - m_bits_offset));
            }
            if (m_offset > m_length)
                m_length = m_offset + (m_bits_offset == 0 ? 0 : 1);
        }
        ...
        {
            CheckWrite(sizeof(Vector3));
            if (m_bits_offset == 0)
            {
                *(Vector3*)m_offset = value;
                m_offset += sizeof(Vector3);
            }
            else
            {
                var temp01 = *(ulong*)&value;
                *(ulong*)m_offset = (byte)(*m_offset & ~(byte.MaxValue << m_bits_offset)) | (temp01 << m_bits_offset);
                m_offset += sizeof(ulong);
                *m_offset = (byte)((byte)(*m_offset & (byte.MaxValue << m_bits_offset)) | (temp01 >> (sizeof(ulong) << 3) - m_bits_offset));
                var temp02 = ((uint*)&value)[2];
                *(uint*)m_offset = (uint)(*m_offset & ~(byte.MaxValue << m_bits_offset)) | (temp02 << m_bits_offset);
                m_offset += sizeof(uint);
                *m_offset = (byte)((byte)(*m_offset & (byte.MaxValue << m_bits_offset)) | (temp02 >> (sizeof(uint) << 3) - m_bits_offset));
            }
            if (m_offset > m_length)
                m_length = m_offset + (m_bits_offset == 0 ? 0 : 1);
        }
        ...
        {
            CheckWrite(sizeof(Vector4));
            if (m_bits_offset == 0)
            {
                *(Vector4*)m_offset = value;
                m_offset += sizeof(Vector4);
            }
            else
            {
                var temp = *(ulong*)&value;
        ...
        {
            CheckWrite(sizeof(Quaternion));
            if (m_bits_offset == 0)
            {
                *(Quaternion*)m_offset = value;
                m_offset += sizeof(Quaternion);
            }
            else
            {
                var temp = *(ulong*)&value;
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        protected static void Copy(byte* src, INetMessage dest, int count)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        protected static void Copy(INetMessage src, byte* dest, int count)
        {
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        protected static void Copy(INetMessage src, INetMessage dest, int count)
        {
