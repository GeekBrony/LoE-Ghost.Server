...
namespace Lidgren.Network
{
	public partial class NetPeer
	{
			...
			string token)
		{
			...
			m_unsentUnconnectedMessages.Enqueue(new Tuple<NetEndPoint, NetOutgoingMessage>(clientExternal, um));
			...
			um.Write(token);
			Interlocked.Increment(ref um.m_recyclingCount);
			m_unsentUnconnectedMessages.Enqueue(new Tuple<NetEndPoint, NetOutgoingMessage>(hostExternal, um));
		}
		...
		internal void HandleNatIntroduction(int ptr)
		{
			...
			m_unsentUnconnectedMessages.Enqueue(new Tuple<NetEndPoint, NetOutgoingMessage>(remoteInternal, punch));
			...
			punch.Write(token);
			Interlocked.Increment(ref punch.m_recyclingCount);
			m_unsentUnconnectedMessages.Enqueue(new Tuple<NetEndPoint, NetOutgoingMessage>(remoteExternal, punch));
		...
		private void HandleNatPunch(int ptr, NetEndPoint senderEndPoint)
		{
			...
			punch.Write(token);
			Interlocked.Increment(ref punch.m_recyclingCount);
			m_unsentUnconnectedMessages.Enqueue(new Tuple<NetEndPoint, NetOutgoingMessage>(senderEndPoint, punch));
		}
	}
}
